...
public:
    ...
    void parameterMenuMoveUp();
   void parameterMenuMoveDown();
    ...
    void setHomeStatusMenuSelected(int idx) {
        ...
        int getHomeStatusMenuCount() const; 
         ...
         int getParameterSelectedIdx() const { return parameterSelectedIdx; }
    void setParameterSelectedIdx(int idx) { parameterSelectedIdx = idx; }
...
private:
    ...
    static constexpr float jogMultipliers[3] = {0.1f, 1.0f, 10.0f};  // ví dụ cấp độ bước nhảy
    ...
    static const int homeStatusMenuCount = 2; // Dòng đang chọn ở PAGE_HOME_STATUS
     int paramCount = AXIS_COUNT*2;
     int getNextUserPage(int current, uint8_t extruderType);
};
